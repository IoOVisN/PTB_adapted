function PT_mlmenu(varargin)
global HARDWARE_MENU
...
% ... vers _SDS  has reconfigured menu functionality for Viseo device(subject screen), Screen Resolution and Refresh Rate
% Screen resolution displays screen, width, height &  Hz, for pixelsize = 32 as accessed by PsychYoolBox 'Screens' function
%
%
%
%
...
lastupdate = 'September 14th, 2010';

mlf = findobj('tag', 'monkeylogicmainmenu');
if ~isempty(mlf) && isempty(gcbo),  %SDS "h = gcbo returns the handle of the graphics object whose callback is executing". 
    figure(mlf);
end
...
validrefresh = [60 70 72 75 85 100 120 240];
...
reloadcondfile = 0;
cfgfile = [];
if ~isempty(varargin),                      %SDS: NB. the initial call to mlmenu from monkeylogic main routine lacks varargin...
...
if isempty(cfgfile),                        %SDS ...hence mlemenu automatically loads the default cfg file (@ L710)
...
if isempty(mlf),    %SDS  draws up control sreen nenu.. till elseif @ L715; then handles menu commands till L3203
    disp(' ')
    disp(' ')
    disp(' ')
    disp(sprintf('<<< PT_MonkeyLogic >>> Revision Date: %s...', lastupdate))
    envOS = getenv('OS');
    ...
    if ~isempty(envOS),
        disp(sprintf('<<< PT_MonkeyLogic >>> Operating System: %s...', envOS))
    end
    if usejava('jvm'),
        disp('<<< PT_MonkeyLogic >>> *** JAVA Virtual Machine is Running ***');
    end
    if ~isempty(envCN),
        disp(sprintf('<<< PT_MonkeyLogic >>> Computer Name: %s...', envCN))
    end
    if ~isempty(envUSER),
        disp(sprintf('<<< PT_MonkeyLogic >>> Logged in as "%s"...', envUSER))
    else
    ...
    if ~isempty(envNOP),
        ...
        if envNOP > 1,
            disp(sprintf('<<< PT_MonkeyLogic >>> Detected %i "%s" processors...', envNOP, envPRC))
        else
            disp(sprintf('<<< PT_MonkeyLogic >>> Detected only %i "%s" processor...', envNOP, envPRC))
        end
    end
    envMVER = version;
    disp(sprintf('<<< PT_MonkeyLogic >>> Matlab version: %s...', envMVER))
    numloops = 1000;
    t = zeros(numloops, 1);
    tic;
    for i = 1:numloops,
    ...
    disp(sprintf('<<< PT_MonkeyLogic >>> Approximate Matlab cycle rate is %4.0f kHz', mrate))
    disp('<<< PT_MonkeyLogic >>> Launching Menu...')
    figure;             %SDS  this launches the display screen; subsequent commands (e.g. set, uicontrol) create contents 
    figbg = [1 0.8 1];      % [.65 .70 .80];
    bgpurple = [.9 .9 .9];  % [.8 .76 .82];
    figx = 1200;
    figy = 750;
    ...
    if scrnx < 1280 || scrny < 768,
        disp('<<< PT_MonkeyLogic >>> Warning: A primary-monitor resolution of at least 1280 x 768 is recommended...')
    end
    ...
    set(gcf, 'closerequestfcn', 'PT_mlmenu; delete(get(0, ''userdata'')); set(0, ''userdata'', ''''); disp(''Closed MonkeyLogic.'')');
%    SDS X_MLVIDEO     
%     mlvideo('mlinit');
%     disp('<<< PT_MonkeyLogic >>> Initialized DirectX / OpenGL interface...')   
    ...
    uicontrol('style', 'edit', 'position', [110 ybase+78 175 20], 'backgroundcolor', [1 1 1], 'tag', 'experimentname', 'string', 'Experiment', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'edit', 'position', [110 ybase+53 175 20], 'backgroundcolor', [1 1 1], 'tag', 'investigator', 'string', envUSER, 'callback', 'PT_mlmenu');
    ...
    h = uicontrol('style', 'pushbutton', 'position', [110 ybase+26 175 22], 'string', cfgname, 'tag', 'configfilename', 'callback', 'PT_mlmenu');
    if ~usejava('jvm'),
        set(h, 'enable', 'off');
    end
    ...
    uicontrol('style', 'pushbutton', 'position', [1115 24 67 33], 'string', 'About...', 'backgroundcolor', bbg, 'tag', 'aboutbutton', 'fontweight', 'normal', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'pushbutton', 'position', [xbase-10 ybase+440 170 40], 'string', 'Load Conditions File', 'tag', 'condfileselect', 'callback', 'PT_mlmenu', 'backgroundcolor', bgpurple, 'fontsize', 10);
    uicontrol('style', 'pushbutton', 'position', [xbase-10 ybase+412 170 23], 'string', 'Edit Conditions File', 'tag', 'editconds', 'callback', 'PT_mlmenu', 'enable', 'off');
    uicontrol('style', 'pushbutton', 'position', [xbase+170 ybase+412 110 30], 'string', 'Save Settings', 'enable', 'off', 'tag', 'savebutton', 'callback', 'PT_mlmenu');
    uicontrol('style', 'pushbutton', 'position', [xbase+170 ybase+450 110 30], 'string', 'Load Settings', 'tag', 'loadbutton', 'callback', 'PT_mlmenu', 'userdata', struct);
    ...
    uicontrol('style', 'edit', 'position', [xbase-205 ybase+346 50 20], 'string', '5000', 'backgroundcolor', [1 1 1], 'tag', 'maxtrials', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'edit', 'position', [xbase-205 ybase+322 50 20], 'string', '1000', 'backgroundcolor', [1 1 1], 'tag', 'maxblocks', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'listbox', 'position', [xbase+312 ybase+38 64 108], 'enable', 'off', 'tag', 'runblocks', 'backgroundcolor', [1 1 1], 'string', '--', 'callback', 'PT_mlmenu', 'max', 2, 'userdata', 1);
    uicontrol('style', 'pushbutton', 'position', [xbase+314 ybase+10 60 22], 'enable', 'off', 'string', 'All', 'callback', 'PT_mlmenu', 'tag', 'allblocks');    
    uicontrol('style', 'text', 'position', [xbase+220 ybase+143 70 20], 'string', 'First block:', 'backgroundcolor', fbg);
    uicontrol('style', 'listbox', 'position', [xbase+225 ybase+100 64 45], 'enable', 'off', 'string', '--', 'backgroundcolor', [1 1 1], 'tag', 'firstblock', 'callback', 'PT_mlmenu')
    ...
    uicontrol('style', 'listbox', 'position', [xbase+13 ybase+10 60 77], 'string', '--', 'enable', 'off', 'backgroundcolor', [1 1 1], 'tag', 'blocklist', 'callback', 'PT_mlmenu', 'userdata', 1);
    %{
    ...
    %}
    uicontrol('style', 'text', 'position', [xbase+120 ybase+23 130 20], 'string', 'Count only correct trials:', 'backgroundcolor', fcolor);
    uicontrol('style', 'checkbox', 'position', [xbase+248 ybase+28 15 15], 'tag', 'countonlycorrect', 'value', 0, 'callback', 'PT_mlmenu');
    %%%%%
    uicontrol('style', 'text', 'position', [xbase+80 ybase+44 170 20], 'string', 'Number of trials to run this block:', 'backgroundcolor', fcolor);
    uicontrol('style', 'edit', 'position', [xbase+248 ybase+47 40 20], 'string', '50', 'enable', 'off', 'backgroundcolor', [1 1 1], 'tag', 'trialsperblock', 'callback', 'PT_mlmenu', 'userdata', 50);
    ...
    uicontrol('style', 'pushbutton', 'position', [xbase+85 ybase+7 100 20], 'string', 'Chart Blocks', 'tag', 'chartblocks', 'callback', 'PT_mlmenu', 'enable', 'off');
    uicontrol('style', 'pushbutton', 'position', [xbase+190 ybase+7 100 20], 'string', 'Apply to all', 'tag', 'applyblocknumstoall', 'callback', 'PT_mlmenu', 'enable', 'off');   
    ...
    uicontrol('style', 'pushbutton', 'position', [585 ybase-1 105 21], 'string', 'Edit', 'tag', 'edittimingfile', 'enable', 'off', 'callback', 'PT_mlmenu');
    
    uicontrol('style', 'text', 'position', [565 ybase-37 145 20], 'string', 'Inter-trial interval:               ms', 'backgroundcolor', fbg);
    uicontrol('style', 'edit', 'position', [652 ybase-32 38 20], 'string', '2000', 'userdata', 2000, 'tag', 'iti', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu');
    
    uicontrol('style', 'frame', 'position', [563 ybase-99 147 61], 'backgroundcolor', 0.9*fbg, 'foregroundcolor', 0.8*fbg);
    uicontrol('style', 'pushbutton', 'position', [566 ybase-66 141 24], 'string', 'MonkeyLogic Latency Test', 'tag', 'mltimetest', 'callback', 'PT_mlmenu', 'enable', 'off');
    uicontrol('style', 'pushbutton', 'position', [566 ybase-95 141 24], 'string', 'Matlab Latency Test', 'tag', 'speedtest', 'callback', 'PT_mlmenu');
    
    ybase = 223;
    uicontrol('style', 'frame', 'position', [300 ybase+112 222 133], 'backgroundcolor', 0.85*figbg, 'foregroundcolor', 0.6*figbg);
    uicontrol('style', 'listbox', 'position', [305 ybase+137 213 105], 'string', '--', 'tag', 'stimnames', 'backgroundcolor', [1 1 1], 'enable', 'off', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'checkbox', 'position', [475 ybase+119 15 15], 'tag', 'savefullmovies', 'value', 1, 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'slider', 'position', [660 ybase+208 60 20], 'enable', 'off', 'tag', 'zoomslider', 'callback', 'PT_mlmenu');
    uicontrol('style', 'text', 'position', [660 ybase+190 60 15], 'string', 'Position', 'backgroundcolor', 0.85*figbg);
    uicontrol('style', 'slider', 'position', [660 ybase+171 60 20], 'enable', 'off', 'tag', 'positionslider', 'callback', 'PT_mlmenu');
    uicontrol('style', 'pushbutton', 'position', [660 ybase+144 60 22], 'string', 'Test', 'tag', 'viewplay', 'enable', 'off', 'callback', 'PT_mlmenu');
    uicontrol('style', 'toggle', 'position', [660 ybase+117 60 22], 'string', 'Process', 'tag', 'preprocessimages', 'enable', 'off', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'popupmenu', 'position', [380 ybase+177 160 20], 'string', erroroptions, 'tag', 'errorlogic', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'popupmenu', 'position', [380 ybase+143 160 20], 'string', condoptions, 'tag', 'condlogic', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu');
    uicontrol('style', 'pushbutton', 'position', [320 ybase+115 220 20], 'string', 'n/a', 'tag', 'condselectfun', 'backgroundcolor', [1 1 1], 'enable', 'off', 'callback', 'PT_mlmenu', 'userdata', '');
    ...
    uicontrol('style', 'popupmenu', 'position', [380 ybase+86 160 20], 'string', blockoptions, 'tag', 'blocklogic', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu');
    uicontrol('style', 'pushbutton', 'position', [320 ybase+57 220 20], 'string', 'n/a', 'tag', 'blockselectfun', 'backgroundcolor', [1 1 1], 'enable', 'off', 'callback', 'PT_mlmenu', 'userdata', '');
    uicontrol('style', 'frame', 'position', [315 ybase+20 230 31], 'backgroundcolor', fbg, 'foregroundcolor', 0.8*fbg);
    uicontrol('style', 'pushbutton', 'position', [320 ybase+25 220 20], 'string', 'Block Change Function', 'tag', 'blockchangefun', 'backgroundcolor', [1 1 1], 'enable', 'on', 'callback', 'PT_mlmenu', 'userdata', '');
    ...
    uicontrol('style', 'edit', 'position', [400 ybase+2 315 20], 'string', '', 'backgroundcolor', [1 1 1], 'tag', 'datafile', 'callback', 'PT_mlmenu');
    uicontrol('style', 'text', 'position', [310 ybase+25 80 20], 'string', 'Subject''s Name:', 'backgroundcolor', f2bg);
    uicontrol('style', 'edit', 'position', [400 ybase+28 145 20], 'string', '', 'backgroundcolor', [1 1 1], 'tag', 'subjectname', 'callback', 'PT_mlmenu', 'userdata', '');
    uicontrol('style', 'text', 'position', [310 ybase+50 80 20], 'string', 'Run-time priority:', 'backgroundcolor', f2bg);
    uicontrol('style', 'popupmenu', 'position', [400 ybase+55 145 20], 'string', {'Normal' 'High' 'Highest'}, 'backgroundcolor', [1 1 1], 'tag', 'priority', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'pushbutton', 'position', [560 ybase+28 155 48], 'string', '', 'callback', 'PT_mlmenu', 'tag', 'runbutton', 'backgroundcolor', [0.9 0.6 0.6], 'enable', 'inactive', 'cdata', imread(pic));
    disp('<<< PT_MonkeyLogic >>> Initialized Task Menu...')
    
    % VIDEO ######################################
%    mlvideo('init');                                   % SDS X_MLVIDEO
%    numdevices = mlvideo('devices');
    numdevices = max(Screen('Screens'));
    if numdevices > 1,
        disp(sprintf('<<< PT_MonkeyLogic >>> Found %i video devices...', numdevices))
    else
        disp('<<< PT_MonkeyLogic >>> Warning: Found only 1 video device...')
    end
%    mlvideo('release');                               % SDS X_MLVIDEO_end
    ...
    ybase = 265;
    uicontrol('style', 'text', 'position', [110 ybase+275 160 20], 'string', '+ PsychToolBox(XGL-free)', 'backgroundcolor', fbg);  %SDS added extra
    uicontrol('style', 'text', 'position', [20 ybase+215 90 20], 'string', 'Screen resolution:', 'backgroundcolor', fbg);
%   HS1 = uicontrol('style', 'popupmenu', 'position', [118 ybase+220 100 20], 'backgroundcolor', [1 1 1], 'string', validsizestrings, 'tag', 'screenres', 'callback', 'PT_mlmenu', 'value', 2, 'userdata', [validxsize' validysize']);
%   HS1 = uicontrol('style', 'popupmenu', 'position', [118 ybase+220 100 20], 'backgroundcolor', [1 1 1], 'string', validscrnstrings, 'tag', 'screenres', 'callback', 'PT_mlmenu', 'value', 2, 'userdata', [validwidth' validheight' validhz']);
    HS1 = screen_resolution(1, -1);  %  (screen, mode;) SDS: matters little which screen is cited here; values are replaced when default_cfg is loaded. 
    uicontrol('style', 'text', 'position', [20 ybase+245 150 20], 'string', 'Video device (subject screen):', 'backgroundcolor', fbg);
    HS2 = uicontrol('style', 'popupmenu', 'position', [178 ybase+250 40 20], 'string', cellstr(num2str((1:numdevices)')), 'backgroundcolor', [1 1 1], 'value', numdevices, 'tag', 'videodevice', 'callback', 'PT_mlmenu', 'userdata', 'numdevices');
    uicontrol('style', 'frame', 'position', [225 ybase+219 55 51], 'backgroundcolor', fbg, 'foregroundcolor', 0.8*fbg);
    uicontrol('style', 'pushbutton', 'position', [230 ybase+224 45 41], 'string', 'Test', 'tag', 'videotest', 'callback', 'PT_mlmenu');
    uicontrol('style', 'text', 'position', [20 ybase+185 100 20], 'string', 'Refresh Rate (Hz):', 'backgroundcolor', fbg, 'horizontalalignment', 'left');
    HS3 = uicontrol('style', 'popupmenu', 'position', [118 ybase+190 45 20], 'string', validrefreshstrings, 'backgroundcolor', [1 1 1], 'tag', 'refreshrate', 'callback', 'PT_mlmenu', 'value', 1, 'userdata', validrefresh);
    uicontrol('style', 'text', 'position', [173 ybase+185 80 20], 'string', 'Buffer Pages:', 'backgroundcolor', fbg, 'horizontalalignment', 'left');
    HS4 = uicontrol('style', 'popupmenu', 'position', [245 ybase+190 34 20], 'string', num2str((1:3)'), 'value', 2, 'backgroundcolor', [1 1 1], 'tag', 'bufferpages', 'callback', 'PT_mlmenu');
    ...
    HS5 = uicontrol('style', 'edit', 'position', [87 ybase+150 40 20], 'backgroundcolor', [1 1 1], 'string', num2str(diagsize), 'tag', 'diagsize', 'callback', 'PT_mlmenu', 'userdata', diagsize);
    ...
    HS6 = uicontrol('style', 'edit', 'position', [231 ybase+150 40 20], 'backgroundcolor', [1 1 1], 'string', num2str(viewdist), 'tag', 'viewdist', 'callback', 'PT_mlmenu', 'userdata', viewdist);
    ...
    uicontrol('style', 'popupmenu', 'position', [120 ybase+84 85 20], 'string', {'None' 'Upper left' 'Upper right' 'Lower right' 'Lower left'}, 'callback', 'PT_mlmenu', 'tag', 'photodiode', 'backgroundcolor', [1 1 1]);
    uicontrol('style', 'text', 'position', [215 ybase+80 30 20], 'string', 'Size:', 'backgroundcolor', fbg);
    uicontrol('style', 'edit', 'position', [250 ybase+84 30 20], 'string', '0', 'backgroundcolor', [1 1 1], 'tag', 'photodiodesize', 'callback', 'PT_mlmenu', 'enable', 'off');
    ...
    uicontrol('style', 'pushbutton', 'position', [130 ybase+52 150 22], 'tag', 'fixationfilename', 'callback', 'PT_mlmenu', 'string', '- Default -', 'userdata', 'DEFAULT');
    ...
    uicontrol('style', 'pushbutton', 'position', [130 ybase+25 150 22], 'tag', 'cursorfilename', 'callback', 'PT_mlmenu', 'string', '- Default -', 'userdata', 'DEFAULT');
    ...
    uicontrol('style', 'edit', 'position', [xbase+45 ybase-22 33 20], 'string', num2str(defaultbgcolor(1)), 'userdata', defaultbgcolor(1), 'backgroundcolor', [1 1 1], 'tag', 'bgred', 'callback', 'PT_mlmenu');
    uicontrol('style', 'edit', 'position', [xbase+78 ybase-22 33 20], 'string', num2str(defaultbgcolor(2)), 'userdata', defaultbgcolor(2), 'backgroundcolor', [1 1 1], 'tag', 'bggreen', 'callback', 'PT_mlmenu');
    uicontrol('style', 'edit', 'position', [xbase+111 ybase-22 33 20], 'string', num2str(defaultbgcolor(3)), 'userdata', defaultbgcolor(3), 'backgroundcolor', [1 1 1], 'tag', 'bgblue', 'callback', 'PT_mlmenu');
    disp('<<< PT_MonkeyLogic >>> Initialized Video Menu...')
    ...
    HS7 = uicontrol('style', 'edit', 'position', [216 ybase+142 30 20], 'backgroundcolor', [1 1 1], 'tag', 'updateinterval', 'string', num2str(defaultupdateinterval), 'userdata', defaultupdateinterval, 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'edit', 'position', [xbase+50 ybase-47 28 20], 'string', num2str(defaulteyecolor(1)), 'userdata', defaulteyecolor(1), 'backgroundcolor', [1 1 1], 'tag', 'eyered', 'callback', 'PT_mlmenu');
    uicontrol('style', 'edit', 'position', [xbase+78 ybase-47 28 20], 'string', num2str(defaulteyecolor(2)), 'userdata', defaulteyecolor(2), 'backgroundcolor', [1 1 1], 'tag', 'eyegreen', 'callback', 'PT_mlmenu');
    uicontrol('style', 'edit', 'position', [xbase+106 ybase-47 28 20], 'string', num2str(defaulteyecolor(3)), 'userdata', defaulteyecolor(3), 'backgroundcolor', [1 1 1], 'tag', 'eyeblue', 'callback', 'PT_mlmenu');
    uicontrol('style', 'text', 'position', [xbase+16 ybase-33 27 20], 'string', 'Size', 'backgroundcolor', figbg);
    uicontrol('style', 'edit', 'position', [xbase+16 ybase-47 30 20], 'string', num2str(defaulteyesize), 'userdata', defaulteyesize, 'backgroundcolor', [1 1 1], 'tag', 'eyesize', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'text', 'position', [xbase+62 ybase-33 57 20], 'string', 'Color RGB', 'backgroundcolor', figbg);
    uicontrol('style', 'edit', 'position', [xbase+50 ybase-47 28 20], 'string', num2str(defaultjoycolor(1)), 'userdata', defaultjoycolor(1), 'backgroundcolor', [1 1 1], 'tag', 'joyred', 'callback', 'PT_mlmenu');
    uicontrol('style', 'edit', 'position', [xbase+78 ybase-47 28 20], 'string', num2str(defaultjoycolor(2)), 'userdata', defaultjoycolor(2), 'backgroundcolor', [1 1 1], 'tag', 'joygreen', 'callback', 'PT_mlmenu');
    uicontrol('style', 'edit', 'position', [xbase+106 ybase-47 28 20], 'string', num2str(defaultjoycolor(3)), 'userdata', defaultjoycolor(3), 'backgroundcolor', [1 1 1], 'tag', 'joyblue', 'callback', 'PT_mlmenu');
    uicontrol('style', 'text', 'position', [xbase+16 ybase-33 27 20], 'string', 'Size', 'backgroundcolor', figbg);
    uicontrol('style', 'edit', 'position', [xbase+16 ybase-47 30 20], 'string', num2str(defaultjoysize), 'userdata', defaultjoysize, 'backgroundcolor', [1 1 1], 'tag', 'joysize', 'callback', 'PT_mlmenu');
    
    ybase = 127;
    uicontrol('style', 'toggle', 'position', [xbase-147 ybase-93 135 23], 'string', 'Cartesian Grid OFF', 'value', 0, 'tag', 'csgrid_cartesian', 'backgroundcolor', [.8 .7 .7], 'callback', 'PT_mlmenu');
    uicontrol('style', 'toggle', 'position', [xbase ybase-93 135 23], 'string', 'Polar Grid OFF', 'value', 0, 'tag', 'csgrid_polar', 'backgroundcolor', [.8 .7 .7], 'callback', 'PT_mlmenu');
    uicontrol('style', 'slider', 'position', [xbase-147 ybase-117 100 18], 'min', 0.01, 'max', 1, 'value', 0.2, 'tag', 'cartesianbrightness', 'callback', 'PT_mlmenu', 'enable', 'off', 'sliderstep', [0.01 0.1]);
    uicontrol('style', 'slider', 'position', [xbase ybase-117 100 18], 'min', 0.01, 'max', 1, 'value', 0.2, 'tag', 'polarbrightness', 'callback', 'PT_mlmenu', 'enable', 'off', 'sliderstep', [0.01 0.1]);
    ...
    uicontrol('style', 'pushbutton', 'position', [45 103 220 20], 'string', 'User Plot Function', 'tag', 'userplotfunction', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu', 'userdata', '');
    ...
    disp('<<< PT_MonkeyLogic >>> Initialized Control-Screen Menu...')
    
    % INPUT / OUTPUT ######################################  
    disp('<<< PT_MonkeyLogic >>> Gathering I/O board info...')
    hwinfo = daqhwinfo;
    AdaptorInfo = ioscan(hwinfo.InstalledAdaptors);
    adaptors = {AdaptorInfo(:).Name};
    disp(sprintf('<<< PT_MonkeyLogic >>> Found %i I/O adaptors:', length(adaptors)))
    ...
    uicontrol('style', 'listbox', 'position', [745 ybase-5 240 110], 'string', adaptors, 'backgroundcolor', [1 1 1], 'tag', 'daq', 'userdata', AdaptorInfo, 'callback', 'PT_mlmenu');
    
    uicontrol('style', 'text', 'position', [995 ybase+105 80 15], 'string', 'Subsystem', 'backgroundcolor', fbg);
    uicontrol('style', 'listbox', 'position', [990 ybase-5 100 110], 'string', AdaptorInfo(1).SubSystemsNames, 'backgroundcolor', [1 1 1], 'tag', 'subsystems', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'pushbutton', 'position', [xbase+180 ybase+6 70 25], 'string', 'Info', 'tag', 'infodaq', 'callback', 'PT_mlmenu');
    uicontrol('style', 'pushbutton', 'position', [xbase+260 ybase+6 70 25], 'string', 'Test', 'tag', 'testdaq', 'callback', 'PT_mlmenu');
    
    uicontrol('style', 'pushbutton', 'position', [xbase+355 ybase+1 70 25], 'string', 'Check', 'tag', 'checkio', 'callback', 'PT_mlmenu');
    ...
    for i = 1: length(iovarnames),
        InputOutput.(iovarnames{i}) = struct;         %SDS  this is where the InputOuput variables(Structures) get created
    ...
    HS8 = uicontrol('style', 'pushbutton', 'position', [xbase+5 ybase+1 70 25], 'string', 'Assign', 'tag', 'setio', 'callback', 'PT_mlmenu');
    HS9 = uicontrol('style', 'pushbutton', 'position', [xbase+85 ybase+1 70 25], 'string', 'Clear', 'tag', 'ioclear', 'callback', 'PT_mlmenu');
    
    uicontrol('style', 'listbox', 'position', [xbase+10 ybase-144 150 132], 'string', iolist, 'tag', 'ioselect', 'userdata', iovarnames, 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu');
    ...
    HS10 = uicontrol('style', 'togglebutton', 'position', [xbase+178 ybase-137 110 21], 'string', 'A-I Duplication OFF', 'tag', 'aiduplication', 'value', 0, 'backgroundcolor', [.8 .7 .7], 'callback', 'PT_mlmenu');
    uicontrol('style', 'text', 'position', [xbase+178 ybase-115 60 20], 'string', 'Frequency:');
    HS11 = uicontrol('style', 'edit', 'position', [xbase+238 ybase-112 50 20], 'string', 1000, 'userdata', 1000, 'tag', 'analoginputfrequency', 'callback', 'PT_mlmenu', 'backgroundcolor', [1 1 1], 'enable', 'off');
    ...
    if ~any(duplicateboard),
        ...
        disp('<<< PT_MonkeyLogic >>> Warning: no duplicate DAQ boards found for Analog Input duplication...')
    else
        disp('<<< PT_MonkeyLogic >>> Detected duplicate DAQ boards: enabling A-I duplication...')
    end
    uicontrol('style', 'pushbutton', 'position', [xbase+299 ybase-138 118 22], 'string', 'Test Analog Inputs', 'tag', 'aitest', 'enable', 'off', 'callback', 'PT_mlmenu');
    
    itype = {'Differential'};
    HS12 = uicontrol('style', 'popupmenu', 'position', [xbase+299 ybase-114 118 23], 'string', itype, 'value', 1, 'tag', 'inputtype', 'enable', 'off', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu');
    ...
    HS13 = uicontrol('style', 'popupmenu', 'position', [xbase+175 ybase+35 60 20], 'string', {'falling' 'rising'}, 'tag', 'strobebitedge', 'enable', 'off', 'backgroundcolor', [1 1 1]);
    uicontrol('style', 'pushbutton', 'position', [xbase+290 ybase+32 120 25], 'string', 'Test Digital Codes', 'tag', 'digcodestest', 'enable', 'off', 'callback', 'PT_mlmenu');
    ...
    if ~exist(codesfile, 'file'),
        ...
        codesfile = 'n/a';
        enablestr = 'off';
        disp('<<< PT_MonkeyLogic >>> Warning: No behavioral codes description file found...')
    else
        enablestr = 'on';
        codestr = defaultcodesfile;
    end
    HS14 = uicontrol('style', 'pushbutton', 'position', [xbase+137 ybase+3 215 20], 'string', codestr, 'tag', 'codesfile', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu', 'userdata', codesfile);
    HS15 = uicontrol('style', 'pushbutton', 'position', [xbase+360 ybase+2 50 22], 'string', 'Edit', 'tag', 'editcodesfile', 'callback', 'PT_mlmenu', 'enable', enablestr);
        
    % Eye and Joystick Calibration Menus *********************
    targetlist = [0 0; 5 5; -5 -5; -5 5; 5 -5];
    ybase = 240;
    uicontrol('style', 'pushbutton', 'position', [862 ybase+27 168 25], 'string', 'Calibrate Eye Position', 'tag', 'calbutton', 'callback', 'PT_mlmenu', 'userdata', []);
    ...
    uicontrol('style', 'checkbox', 'position', [1078 ybase+30 20 20], 'tag', 'useraw', 'callback', 'PT_mlmenu', 'backgroundcolor', fbg, 'userdata', []);
    uicontrol('style', 'pushbutton', 'position', [745 ybase+27 110 25], 'string', 'Import Eye Cal', 'tag', 'loadeyetransform', 'callback', 'PT_mlmenu');

    xbase = 755;
    ybase = ybase - 32;
    uicontrol('style', 'pushbutton', 'position', [862 ybase+27 168 25], 'string', 'Calibrate Joystick Position', 'tag', 'joycalbutton', 'callback', 'PT_mlmenu', 'userdata', []);
    ...
    uicontrol('style', 'checkbox', 'position', [1078 ybase+30 20 20], 'tag', 'userawjoy', 'callback', 'PT_mlmenu', 'backgroundcolor', fbg, 'userdata', 1);
    uicontrol('style', 'pushbutton', 'position', [745 ybase+27 110 25], 'string', 'Import Joy Cal', 'tag', 'loadjoytransform', 'callback', 'PT_mlmenu');

    % Eye Drift Correction Menu *****************
    ybase = ybase - 15;
    uicontrol('style', 'frame', 'position', [xbase-10 ybase-112 192 147], 'backgroundcolor', fbg);
    uicontrol('style', 'toggle', 'position', [xbase-1 ybase+6 174 22], 'string', 'Eye Drift Correction OFF', 'tag', 'eyeadjust', 'value', 0, 'backgroundcolor', [.8 .7 .7], 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'edit', 'position', [xbase+117 ybase-19 30 20], 'string', '50', 'userdata', 50, 'tag', 'adjustfraction', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu', 'enable', 'off');
    ...
    uicontrol('style', 'edit', 'position', [xbase+117 ybase-44 30 20], 'string', '1', 'userdata', 1, 'tag', 'fixdegrees', 'backgroundcolor', [1 1 1], 'foregroundcolor', [0 0 0], 'callback', 'PT_mlmenu', 'enable', 'off');
    uicontrol('style', 'text', 'position', [xbase+72 ybase-73 100 20], 'string', 'Fix Time:            ms', 'backgroundcolor', fbg, 'horizontalalignment', 'left');
    uicontrol('style', 'edit', 'position', [xbase+117 ybase-69 30 20], 'string', '100', 'userdata', 100, 'tag', 'fixtime', 'backgroundcolor', [1 1 1], 'foregroundcolor', [0 0 0], 'callback', 'PT_mlmenu', 'enable', 'off');
    ...
    uicontrol('style', 'edit', 'position', [xbase+117 ybase-94 30 20], 'string', '15', 'userdata', 15, 'backgroundcolor', [1 1 1], 'tag', 'smoothsigma', 'callback', 'PT_mlmenu', 'enable', 'off');
    uicontrol('style', 'frame', 'position', [xbase-1 ybase-72 56 50], 'backgroundcolor', fbg, 'foregroundcolor', 0.8*fbg);
    uicontrol('style', 'pushbutton', 'position', [xbase+3 ybase-67 48 40], 'string', 'Test', 'tag', 'eyeadjusttest', 'callback', 'PT_mlmenu', 'enable', 'off');
    ...
    uicontrol('style', 'checkbox', 'position', [xbase+133 ybase-108 12 12], 'tag', 'firsttargetonly', 'callback', 'PT_mlmenu', 'backgroundcolor', fbg, 'userdata', 1);
    ...
    uicontrol('style', 'toggle', 'position', [xbase+30 ybase+6 174 22], 'string', 'Remote Alerts OFF', 'tag', 'alertsandupdates', 'backgroundcolor', [.8 .7 .7], 'callback', 'PT_mlmenu');
    uicontrol('style', 'text', 'position', [xbase+20 ybase-25 80 20], 'string', 'Error Alerts:', 'backgroundcolor', fbg, 'horizontalalignment', 'left');
    uicontrol('style', 'checkbox', 'position', [xbase+85 ybase-18 15 15], 'value', 1, 'enable', 'off', 'tag', 'au_errorcheck', 'callback', 'PT_mlmenu');
    uicontrol('style', 'text', 'position', [xbase+120 ybase-25 80 20], 'string', 'Block Updates:', 'backgroundcolor', fbg, 'horizontalalignment', 'left');
    uicontrol('style', 'checkbox', 'position', [xbase+198 ybase-18 15 15], 'value', 1, 'enable', 'off', 'tag', 'au_blockcheck', 'callback', 'PT_mlmenu');
    ...
    uicontrol('style', 'checkbox', 'position', [xbase+179 ybase-45 15 15], 'value', 0, 'enable', 'off', 'tag', 'au_userdefinedcheck', 'callback', 'PT_mlmenu');
    uicontrol('style', 'pushbutton', 'position', [xbase+15 ybase-70 203 20], 'string', 'n/a', 'tag', 'au_userdefinedcritfunction', 'enable', 'off', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu', 'userdata', '');
    uicontrol('style', 'pushbutton', 'position', [xbase+15 ybase-103 203 20], 'string', 'n/a', 'tag', 'au_function', 'enable', 'off', 'backgroundcolor', [1 1 1], 'callback', 'PT_mlmenu', 'userdata', '');
    
    disp('<<< PT_MonkeyLogic >>> Initialized I/O Menu...')
    
    %%% PULL-DOWN MENUS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    filemenu = uimenu('label', 'File');
    uimenu(filemenu, 'label', 'Load Conditions File', 'tag', 'menubar_condfileselect', 'callback', 'PT_mlmenu', 'accelerator', 'f');
    uimenu(filemenu, 'label', 'Load Configuration', 'tag', 'menubar_loadbutton', 'callback', 'PT_mlmenu');
    uimenu(filemenu, 'label', 'Save Configuration', 'tag', 'menubar_savebutton', 'callback', 'PT_mlmenu', 'enable', 'off', 'accelerator', 's');
    ...
    uimenu(editmenu, 'label', 'Edit Conditions File', 'tag', 'menubar_editconds', 'callback', 'PT_mlmenu', 'enable', 'off', 'accelerator', 'e');
    uimenu(editmenu, 'label', 'Edit Timing File', 'tag', 'menubar_edittimingfile', 'callback', 'PT_mlmenu', 'enable', 'off');
    HS16 = uimenu(editmenu, 'label', 'Edit Behavioral Codes File', 'tag', 'menubar_editcodesfile', 'callback', 'PT_mlmenu');
    ...
    uimenu(iomenu, 'label', 'Board Info', 'tag', 'menubar_infodaq', 'callback', 'PT_mlmenu');
    uimenu(iomenu, 'label', 'Oscilloscope', 'tag', 'menubar_testdaq', 'accelerator', 'o', 'callback', 'PT_mlmenu');
    uimenu(iomenu, 'label', 'Import Eye Calibration Matrix', 'tag', 'menubar_loadeyetransform', 'callback', 'PT_mlmenu', 'separator', 'on');
    uimenu(iomenu, 'label', 'Import Joystick Calibration Matrix', 'tag', 'menubar_loadjoytransform', 'callback', 'PT_mlmenu');
    HS17 = uimenu(iomenu, 'label', 'Invert Reward Polarity', 'tag', 'menubar_rewardpolarity', 'callback', 'PT_mlmenu', 'separator', 'on');
    
    diagnosticsmenu = uimenu('label', 'Diagnostics');
    uimenu(diagnosticsmenu, 'label', 'Video Test', 'tag', 'menubar_videotest', 'callback', 'PT_mlmenu', 'accelerator', 'v');
    uimenu(diagnosticsmenu, 'label', 'Test Stimulus Object', 'tag', 'menubar_viewplay', 'callback', 'PT_mlmenu', 'enable', 'off', 'accelerator', 't');
    uimenu(diagnosticsmenu, 'label', 'I/O Check', 'tag', 'menubar_checkio', 'callback', 'PT_mlmenu', 'separator', 'on', 'accelerator', 'i');
    uimenu(diagnosticsmenu, 'label', 'Test Digital Codes', 'tag', 'menubar_digcodestest', 'callback', 'PT_mlmenu', 'enable', 'off', 'accelerator', 'd');
    uimenu(diagnosticsmenu, 'label', 'Analog Sampling Rate Test', 'tag', 'menubar_aitest', 'callback', 'PT_mlmenu', 'enable', 'off');
    uimenu(diagnosticsmenu, 'label', 'Eye-Drift Saccade/Fixation Detection', 'tag', 'menubar_eyeadjusttest', 'callback', 'PT_mlmenu');
    uimenu(diagnosticsmenu, 'label', 'Remote Alerts Test', 'tag', 'menubar_alertstest', 'callback', 'PT_mlmenu', 'separator', 'on');
    uimenu(diagnosticsmenu, 'label', 'MonkeyLogic Latency Test', 'tag', 'menubar_mltimetest', 'callback', 'PT_mlmenu', 'separator', 'on', 'enable', 'off');
    uimenu(diagnosticsmenu, 'label', 'MATLAB Latency Test', 'tag', 'menubar_speedtest', 'callback', 'PT_mlmenu');
    
    advancedmenu = uimenu('label', 'Advanced');
    uimenu(advancedmenu, 'label', 'Preload Video Data', 'tag', 'menubar_preload', 'callback', 'PT_mlmenu');
    uimenu(advancedmenu, 'label', 'Enable Mouse/System Keys', 'tag', 'menubar_mlhelper', 'callback', 'PT_mlmenu');
    uimenu(advancedmenu, 'label', 'Use Personal Hardware Settings', 'tag', 'menubar_personalhardware', 'callback', 'PT_mlmenu', 'enable', 'off');
    
    helpmenu = uimenu('label', 'Help');
    uimenu(helpmenu, 'label', 'Online Help', 'callback', 'web(''http://www.monkeylogic.net'', ''-browser'')', 'accelerator', 'h');
    uimenu(helpmenu, 'label', 'About...', 'tag', 'menubar_aboutbutton', 'callback', 'PT_mlmenu');
    if usejava('jvm'),
        disp('<<< PT_MonkeyLogic >>> Initialized drop-down menus...')
    end
...
%     ioclear
%     aiduplication
%     analoginputfrequency
%     inputtype
%     strobebitedge
%     codesfile
%     editcodesfile
%     menubar_editcodesfile
%     menubar_rewardpolarity
    
    HARDWARE_MENU = [HS1 HS2 HS3 HS4 HS5 HS6 HS7 HS8 HS9 HS10 HS11 HS12 HS13 HS14 HS15 HS16 HS17];
    
    if exist(cfgfile, 'file'),
        loadcfg(cfgfile);             %SDS - subroutine, L3612
        update_minicontrolscreen;
    else %WA: if doesn't exist, create config file...
        savecfg;                        %SDS - subroutine, L4114
    end
    disp('<<< PT_MonkeyLogic >>> Ready.')
    disp(' ')
...
elseif ismember(gcbo, get(findobj('tag', 'monkeylogicmainmenu'), 'children')) || reloadcondfile || ismember(get(gcbo, 'parent'), get(findobj('tag', 'monkeylogicmainmenu'), 'children')),
    ...
    switch callertag,
        ...
        case 'condfileselect',  %SDS to L880: this is the 'Load Conditions File'  button
            ...
            [Conditions cerror] = PT_load_conditions(fullfile);
            if ~isempty(cerror),
            ...
            if ~iscell(Conditions), %WA: no userloop: regular conditions file
                userloop = 0;
                ...
                UniqueTaskObjects = PT_sort_taskobjects(Conditions);
            ...
            %WA: update settings if config file already exists...
            cfgfile = [MLPrefs.Directories.ExperimentDirectory cfgfile];
            if exist(cfgfile, 'file'),
                loadcfg(cfgfile);    
            end
            ...
            if userloop, %WA: these gui changes must happen *after* loadcfg, which may otherwise over-ride some lines below                
        ...
        case 'editconds',
            ...
            mlmessage('');
            savecfg;
            PT_mlmenu(-1); %reload conditions file to update menu parameters with any changes
            
        case 'savebutton',
%           savecfg;            %SDS: Added varargin(0) this becomes hardware_flag =0, and acts to disable a call to overwrite_hardware_cfg
            savecfg(0);         %SDS .. that will otherwise retain existing screen settings (refer to subroutine @ L3616)
        ...
        case 'loadbutton', %SDS: this is the Load Settings button
...
%             f = max(find(cfgfile == filesep)); 
%             cfgname = cfgfile(f+1:length(cfgfile));
            set(findobj(gcf, 'tag', 'configfilename'), 'string', filename);
        ...
        case 'personalhardware',
            ...
            if strcmpi(p, 'off'),
                set(gcbo, 'checked', 'on');
                savecfg(1);     %SDS: 1 becomes 'hardware_flag' =1, permits overwrite hardware_cfg, that retains screen settings of existing cfg file
            else
                set(gcbo, 'checked', 'off');
                savecfg(0);     %SDS: 0 becomes 'hardware_flag' =0, and prevents overwrite hardware_cfg
            end
        ...
        case 'mltimetest',
            ...
            if ~strcmp(nullstr, '--'),
                ...
                condfile = get(findobj(gcf, 'tag', 'condfileselect'), 'userdata');
                testflag = 1;
                Result = PT_monkeylogic(condfile, datafile, testflag);
        ...
        case 'screenres',
            ...
            numdevices = length(get(findobj(gcf, 'tag', 'videodevice'), 'string'));     %SDS added extra
            Di = get(findobj(gcf, 'tag', 'videodevice'), 'value');                      %SDS added extra
            resval = get(gcbo, 'value');                                                %SDS added extra
            validscreensizes = get(gcbo, 'userdata');   %SDS: 'screenres' userdata is a matrix with 3n columns (where n = number of available screens)
            validwidth = validscreensizes(:, Di);                                   %SDS: .. 1st column is screen(1) widths, 2nd is screen(2) widths, 3rd is screen(3)widths
            validheight = validscreensizes(:, Di + numdevices);                     %SDS: .. 1+nth column is screen(1) heights, etc...
            validhz = validscreensizes(:, Di + 2*numdevices);                       %SDS: .. 1+2nth column is screen(1) hzs, etc...
            xsize = validwidth(resval);
            ysize = validheight(resval);
            res_hz = validhz(resval);  
            validrefresh = get(findobj(gcf, 'tag', 'refreshrate'), 'userdata');
            set(findobj(gcf, 'tag', 'refreshrate'), 'value', find(validrefresh == res_hz));
...
%             ysize = validres(reschoice, 2);
        ...
        case 'videodevice',
            ...
            screenPick = get(findobj(gcf, 'tag', 'videodevice'), 'value');
            screen_resolution(screenPick,0);
%            HS1 = screen_resolution(screenPick,0);
%           HARDWARE_MENU = [HS1 HS2 HS3 HS4 HS5 HS6 HS7 HS8 HS9 HS10 HS11 HS12 HS13 HS14 HS15 HS16 HS17];  % probably not necessary
            set(findobj(gcf, 'tag', 'savebutton'), 'enable', 'on');
            set(findobj(gcf, 'tag', 'menubar_savebutton'), 'enable', 'on');
...
%                     mlvideo('copybuffer', videodevice, buffer(i), testpattern);  % SDS - calls XGLCOPYBUFFER => Copies pixels to offscreen memory.
%                 end
...
%             if usejava('jvm'),                                                %SDS amend:  java errors are caused by XGL; 
...
%             end
            ...
            if ~strcmp(nullstr, '--'),
                ...
                PT_monkeylogic(condfile, datafile, testflag);  %SDS - calls monkeylogic !!!   % SDS X_MLVIDEO   
            end
            set(gcbo, 'hittest', 'on');
            mlmessage('Done.');
        ...
        case 'viewplay',
            ...
            if strcmpi(t, 'fix') || strcmpi(t, 'pic') || strcmpi(t, 'dot') || strcmpi(t, 'crc') || strcmpi(t, 'sqr') || strcmpi(t, 'gen'),
                fprintf('XGL free mode: invalid task object type %s /n', t)
                return
%  SDS X_MLVIDEO  
...
% SDS X_MLVIDEO_end   
            ...
            elseif strcmpi(t, 'mov'), %movie
                fprintf('XGL free mode: invalid task object type %s (movie) /n', t)
                return
% SDS X_MLVIDEO_end   
...
% SDS X_MLVIDEO_end   
        ...
        case 'setio',       % SDS till 2960 - operates the "Assign" button
...
%     zoomstartval = 0; %temp
%     bgcolor = [0 0 0]; %temp
elseif strcmpi(TOB.Type, 'gab')||strcmpi(TOB.Type, 'msk')||strcmpi(TOB.Type, 'fxc'),      %  SDS ADDED EXTRA
    minzoom = 0; % SDS have not yet worked out exactly what all these do:
    maxzoom = 1; % SDS - using above (SDS commented) values for 'gen' objects...
    minpos = 0;         %SDS - etc.
    maxpos = 1;         %SDS - etc.
    zoomstartval = 0;   %SDS - etc.
    bgcolor = [0 0 0];  %SDS - etc.
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function MLConfig_combine = overwrite_hardware_cfg(MLConfig_all, hardware_cfgfile, set_personal)
load(hardware_cfgfile);
MLConfig_combine = MLConfig_all;
MLConfig_combine.ScreenX = MLConfig.ScreenX;
MLConfig_combine.ScreenY = MLConfig.ScreenY;
MLConfig_combine.VideoDevice = MLConfig.VideoDevice;
MLConfig_combine.RefreshRate = MLConfig.RefreshRate;
MLConfig_combine.BufferPages = MLConfig.BufferPages;
MLConfig_combine.DiagonalScreenSize = MLConfig.DiagonalScreenSize;
MLConfig_combine.ViewingDistance = MLConfig.ViewingDistance;
MLConfig_combine.UpdateInterval = MLConfig.UpdateInterval;
MLConfig_combine.InputOutput = MLConfig.InputOutput;
MLConfig_combine.BehavioralCodesTextFile = MLConfig.BehavioralCodesTextFile;
%set_personal: 0 -> 0, 1 -> 1, 2 -> All, 3 -> Hardware
if set_personal == 2,
    MLConfig_combine.PersonalHardwareSettings = MLConfig_all.PersonalHardwareSettings;
elseif set_personal == 3,
    MLConfig_combine.PersonalHardwareSettings = MLConfig.PersonalHardwareSettings;
else
    MLConfig_combine.PersonalHardwareSettings = set_personal;
end
...
load(cfgfile);
[p f] = fileparts(cfgfile);
if strcmpi(f,'default_cfg'),
    menu = findobj(gcf, 'tag', 'menubar_personalhardware');
    set(menu, 'enable', 'off'); %SDS: greyouts Advanced/Use Personal Hardware (top menubar)
    set(menu, 'checked', 'on'); %SDS: 'ticks' Advanced/Use Personal Hardware (top menubar)
    for i = 1:length(HARDWARE_MENU),
        setting = HARDWARE_MENU(i);
        set(setting, 'enable', 'on');
    end
elseif ~isfield(MLConfig, 'PersonalHardwareSettings') || MLConfig.PersonalHardwareSettings, %#ok<NODEF>
    menu = findobj(gcf, 'tag', 'menubar_personalhardware');
    set(menu, 'enable', 'on');
    set(menu, 'checked', 'on');
    for i = 1:length(HARDWARE_MENU),
    ...
    menu = findobj(gcf, 'tag', 'menubar_personalhardware');
    set(menu, 'enable', 'on');
    set(menu, 'checked', 'off');  %SDS: i.e. not using any menu settings for videodevice, resolution etc.
    for i = 1:length(HARDWARE_MENU),
        setting = HARDWARE_MENU(i);
        set(setting, 'enable', 'off');   %SDS  disables (i.e. greys-out) all  menu items in HARDWARE list (refer to line 687)
    end
    MLPrefs.Directories = getpref('MonkeyLogic', 'Directories');
    hardware_cfgfile = [MLPrefs.Directories.ExperimentDirectory 'default_cfg.mat'];
    MLConfig = overwrite_hardware_cfg(MLConfig,hardware_cfgfile,2); %SDS:  Substitute with settings from MLogic main directory default config file 
end
...
set(findobj(gcf, 'tag', 'videodevice'), 'value', MLConfig.VideoDevice); %SDS: 'value'  is the index of the items allocated by 'string' @L370

Di = MLConfig.VideoDevice;  
numdevices = length(get(findobj(gcf, 'tag', 'videodevice'), 'string'));
validscreensizes = get(findobj(gcf, 'tag' ,'screenres'), 'userdata');   %SDS: userdata is a matrix with 3n columns (where n = number of available screens)
validwidth = validscreensizes(:, Di);                                  %SDS: .. 1st column is screen(1) widths, 2nd is screen(2) widths, 3rd is screen(3)widths
validheight = validscreensizes(:, Di + numdevices);                     %SDS: .. 1+nth column is screen(1) heights, etc...
validhz = validscreensizes(:, Di + 2*numdevices);                       %SDS: .. 1+2nth column is screen(1) hzs, etc...

% Bout = [MLConfig.ScreenX MLConfig.ScreenY] %SDS temp diagnostic
% Aout = find(validwidth == MLConfig.ScreenX & validheight == MLConfig.ScreenY & validhz == MLConfig.RefreshRate)  %SDS temp diagnostic
screen_resolution(Di, 0) %SDS: this subroutine calls up the appropriate set of multiple screen resolution data, for the default configuration screen number 
set(findobj(gcf, 'tag', 'screenres'), 'value', find(validwidth == MLConfig.ScreenX & validheight == MLConfig.ScreenY & validhz == MLConfig.RefreshRate));  %SDS AMEND   
set(findobj(gcf, 'tag', 'viewdist'), 'string', num2str(MLConfig.ViewingDistance));                                        %SDS... 'value' is the index of the items allocated by 'string' @L368;
set(findobj(gcf, 'tag', 'diagsize'), 'string', num2str(MLConfig.DiagonalScreenSize));                                     %SDS... videodevice is set @ L3939 (or @ +184 lines) 

% numdevices = length(get(findobj(gcf, 'tag', 'videodevice'), 'string')); %% this line now appears above @ L3696 (or -244 lines)
...
MLConfig.VideoDevice = get(findobj(gcf, 'tag', 'videodevice'), 'value');  %SDS: 'value' gives the selected position in the list - but OK here as the list is always just 1,2,(3,4,5...)
validrefresh = get(findobj(gcf, 'tag', 'refreshrate'), 'userdata');
...
validheight = validscreensizes(:, Di + numdevices);                     %SDS: .. 1+nth column is screen(1) heights, etc...
validhz = validscreensizes(:, Di + 2*numdevices);                       %SDS: .. 1+2nth column is screen(1) hzs, etc...
MLConfig.ScreenX = validwidth(resval);
MLConfig.ScreenY = validheight(resval); 
...
cfgfile = [MLPrefs.Directories.ExperimentDirectory cfgfile]; 
if (hardware_flag == -1 && ~MLConfig.PersonalHardwareSettings) || hardware_flag == 1,  %SDS the call  savecfg(0) sets hardware_flag = 0, and prevents overwrite_hardware_cfg from operating
    MLConfig = overwrite_hardware_cfg(MLConfig,cfgfile,2); %#ok<NASGU>      %SDS:  subroutine @ L3616: generates an error message if there is not a 'default_cfg' file available;
end                                                                         %SDS:  ..effectively, the routine preserves existing values of screen settings 
MLConfig.InputOutput.Configuration.AnalogInputType = 'Differential';    %SDS  ADDED LINES from loadcfg (- these subfields needed to write bhv file header, called from monkeylogic)
MLConfig.InputOutput.Configuration.AnalogInputFrequency = 1000;         %SDS  ADDED LINES from loadcfg
MLConfig.InputOutput.Configuration.RewardPolarity = 1;                  %SDS  ADDED LINES from loadcfg
try
    save(cfgfile, 'MLConfig');
    if (hardware_flag == -1 && ~MLConfig.PersonalHardwareSettings) || hardware_flag == 1, 
        disp('... Updating configuration file, but preserving subject screen settings & Input/Output settings...') %SDS added extra
    else
        disp('... Updating configuration file with new subject screen settings ...') %SDS added extra
    end
catch
...
function HS = screen_resolution(pickScreen, mode)  %SDS ADDED EXTRA

persistent  validscrnstrings % validwidth validheight validhz

switch mode
    case -1        %   initialization 
        [validwidth validheight validhz] = screen_settings(32);   %  validwidth, etc: 1 row for each screen; as many columns as max number of settings

        numvalids = length(validwidth);
        Sn = max(Screen('Screens'));            
        validscrnstrings = cell(Sn, numvalids);
        for j = 1:Sn
            for i = 1:numvalids,
                validscrnstrings{j,i} = sprintf('%i x %i:  %iHz', validwidth(j,i), validheight(j,i), validhz(j,i));
            end
        end
        
        ybase = 265;
        HS = uicontrol('style', 'popupmenu', 'position', [118 ybase+220 100 20], 'backgroundcolor', [1 1 1], 'string', validscrnstrings(Sn,:), 'tag', 'screenres', 'callback', 'PT_mlmenu', 'value', 2, 'userdata', [validwidth' validheight' validhz']);
        % Default screen = 2 (or max screen) but this immediately replaced by values in default Configuration file (loaded @ L 
    case 0
%         numvalids = length(validwidth);
%         validscrnstrings = cell(1, numvalids);
%         for i = 1:numvalids,
%             validscrnstrings{i} = sprintf('%i x %i:  %iHz', validwidth(pickScreen,i), validheight(pickScreen,i), validhz(pickScreen,i));
%         end
        L_validscrnstrings = length(validscrnstrings) %SDS:  temp; (==0 if validscrnstrings has 'disappeared');
        if L_validscrnstrings ==0                     %SDS: a wierd, sporadic, untraceable bug acts as if to cause validscrnstrings to drop out of persistent memory;
            screen_resolution(pickScreen, 1)         %SDS ..to rectify this, reinstall validscrnstrings by having routine call itself in mode 1 
        end
        set(findobj(gcf, 'tag', 'screenres'), 'string', validscrnstrings(pickScreen,:));
        
    case 1 %SDS 'validscrnstrings' restoration mode, (only ever called from case 0, above)
        [validwidth validheight validhz] = screen_settings(32);   %  validwidth, etc: 1 row for each screen; as many columns as max number of settings

        numvalids = length(validwidth);
        Sn = max(Screen('Screens'));  %SDS temp;x 
        disp('***WARNING: validscrnstrings HAS GOT LOST, AGAIN!*** - - But never mind, auto-repair in operation - - ')
        validscrnstrings = cell(Sn, numvalids);
        for j = 1:Sn
            for i = 1:numvalids,
                validscrnstrings{j,i} = sprintf('%i x %i:  %iHz', validwidth(j,i), validheight(j,i), validhz(j,i));
...
function [Widths Heights Hzs] = screen_settings(spec_pixsize)   %SDS ADDED EXTRA

% Substitute function for interrogating valid screen settings and returning
% ... info to suit existing design of MonkeyLogic.
...
Sn = max(Screen('Screens'));
Widths(Sn,166) = zeros;   % preallocate 166 (more than large enough...) trim later
