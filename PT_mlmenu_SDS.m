function PT_mlmenu_SDS(varargin)
...
% Screen resolution displays screen, width, height &  Hz, for pixelsize = 32ccessed by PsychYoolBox 'Screens' function
%
%
%
%
...
[validwidth validheight validhz] = screen_settings(32); %SDS AMEND
pickScreen = 1; % SDS: arbitrarily pick a screen to get started; values are replaced by those loaded from default configuration file before user ever gets to see a display...
...
%     menubar_rewardpolarity
    ...
    if exist(cfgfile, 'file'),
        loadcfg(cfgfile);      %SDS temp         %SDS - subroutine, L3612
...
elseif ismember(gcbo, get(findobj('tag', 'monkeylogicmainmenu'), 'children')) || reloadcondfile || ismember(get(gcbo, 'parent'), get(findobj('tag', 'monkeylogicmainmenu'), 'children')),
    ...
    switch callertag,
        ...
        case 'condfileselect',  %SDS to L880
            ...
            set(gcf, 'pointer', 'arrow');
            set(findobj(gcf, 'tag', 'screenres'), 'enable', 'on');    %SDS added extra; switch these back on (turned off by 'loadcfg' called @L836)
            set(findobj(gcf, 'tag', 'videodevice'), 'enable', 'on');  %SDS added extra
            set(findobj(gcf, 'tag', 'refreshrate'), 'enable', 'on');  %SDS added extra
            set(findobj(gcf, 'tag', 'bufferpages'), 'enable', 'on');  %SDS added extra
            set(findobj(gcf, 'tag', 'editconds'), 'enable', 'on');    %SDS added extra
            set(findobj(gcf, 'tag', 'savebutton'), 'enable', 'on');   %SDS added extra
...
elseif ~isfield(MLConfig, 'PersonalHardwareSettings') || MLConfig.PersonalHardwareSettings, %#ok<NODEF>
    ...
    MLConfig = overwrite_hardware_cfg(MLConfig,hardware_cfgfile,2);
end
...
validwidth = validscreensizes(:, 1 + (Di-1)*numdevices);               %SDS: .. 1st column is screen(1) widths, 2nd is screen(1) heights, 3rd is screen(1)Hz
validheight = validscreensizes(:, 2 + (Di-1)*numdevices);              %SDS: .. 4th column is screen(2) widths, etc...
validhz = validscreensizes(:, 3 + (Di-1)*numdevices);
validrefresh = get(findobj(gcf, 'tag', 'refreshrate'), 'userdata');
...
% set(findobj(gcf, 'tag', 'screenres'), 'value', find(validxsize == MLConfig.ScreenX & validysize == MLConfig.ScreenY)); %SDS: sets Screen resolution display box to setting stored in loaded Config file
...
Sn = max(Screen('Screens')); % PsychToolBox function
...
for  i = 1:Sn
    ...
    Widths(i,LpS) = 0;           % Clumsy means to force MATLAB to allow the subsequent line, without generating an error !!
    Widths(i,:) = width(pS);
    
    Heights(i,LpS) = 0;          % Clumsy etc
    Heights(i,:) = height(pS);
    
    Hzs(i,LpS) = 0;              % Clumsy etc
    Hzs(i,:) = hz(pS);
