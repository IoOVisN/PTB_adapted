function [Conditions, cerror] = PT_load_conditions(varargin)
%SYNTAX:
...
% SDS: load_conditions is called first by mlmenu, but then again by monkeylogic;
% SDS: mlmenu provides only the identity of the conditions file to monkeylogic
% SDS: ..all the other conditions file descriptors/parameters are reloaded.
% SDS: Hence, the specific TaskObject fieldnames defined below are used by 'sort_taskobjects' (called by mlmenu)
% SDS: - but the fieldnames may be renamed when the TaskObject descriptors defined here are used in the creation
% SDS ..of the totally separate (but similarly named) TaskObject structure within the monkeylogic main routine.
...
h = parse_object(header);       %SDS  'parse_object@ is a separate main MLogic directory routine
...
numconds = 0;
txt = fgetl(fid);   %SDS - reads in text of conditions file line by line; hence 'txt' == one entire line
if txt == -1,
...
sinebank = [];
while txt ~= -1,    %SDS  end @ L701;  L700 is 'txt = fgetl(fid)'  that reads in the next line...
    if txt(1) ~= '%' && ~isempty(deblank(txt)), %allows for comments if first character in a line is a "%" and for blank lines
        numconds = numconds + 1;
        p = parse_object(txt);      %SDS  'parse_object' is a separate main MLogic directory routine
                                    %SDS  ...it returns a text array of all items, as separated by tabs, in the current line of the conditions file
        ...
        % Read Task Objects
        clear TaskObject
%       TaskObject(1:100) = struct('Type', '', 'Name', '', 'RawText', '', 'FunctionName', '', 'Xpos', [], 'Ypos', [], 'Xsize', [], 'Ysize', [], 'Color', [], 'Radius', [], 'FillFlag', [], 'WaveForm', [], 'Freq', [], 'NBits', [], 'OutputPort', []);
        TaskObject(1:100) = struct('Type', '', 'Name', '', 'Class', '','RawText', '', 'FunctionName', '', 'Radius', [], 'Angle', [],'Xpos', [], 'Ypos', [], 'FP1', [], 'FP2', [], 'FP3', [],'FP4', [],'Color', [], 'WaveForm', [], 'Freq', [], 'NBits', [], 'OutputPort', []);
        for obnum = 1:(length(p))-nonTaskCols,  %SDS ends @ L693
            object = p{columns(obnum+length(column_order))};  %SDS  this is a single item, the first object item, e.g. 'fix(0,0)'
            ...
            TaskObject(obnum).Type = obtype;  %SDS this will be program name e.g. 'GAB'  (or  'gab' after lower command)
            ...
            if ~isempty(op),
                ...
                attributes = parse_object(object(op+1:cp-1), double(',')); %SDS  returns (i.e. as text)the characters of the currrent object item, as separated by commas , e.g. '0' '0'
                numatt = length(attributes);
                
                % SDS - HERE define how numeric, matrix or text parameters are translated into particular fields of the structure TaskObject.
                % The first second third etc comma-separated items within brackets of the conditions file are presented here as:-
                %     attributes{1}, attributes{2}  attributes{3}  etc    % NB.. use of curly brackets in attributes{1}   !!
                
                % Write fresh code for each new PsychToolBox program:-
                % If single numeric parameter...  TaskObject(obnum).Field = str2double(attributes{1})
                % If matrix parameter...  TaskObject(obnum).Field = eval(attributes{1})
                % If text parameter... TaskObject(obnum).Field = attributes{1}
                
                % NB. TaskObject(obnum) collates the TaskObject descriptors for each object within a single condition
                % NB. At the end of the 'if' loop (initiated @ L115) the condition data is appended as Conditions(condnum).TaskObject
                
                
                if strcmp(obtype, 'gab'), 
                    %Gabor array - syntax: GAB(class, radius, angle, target position, level)
                    %                e.g.  GAB(ori,320,135,1,7) 
                    if numatt < 5,
                        cerror = sprintf('Must have 5 attributes (class, radius, angle, target position, level) for GAB task object #%i, in condition #%i', obnum, condnum);
                        return
                    end
                 
                    class = attributes{1};
                    possibles = {'ori', 'spf', 'dir', 'siz', 'col', 'int'};
                    
                    if ~find(strcmpi(class, possibles)==1)
                        cerror = sprintf('First attribute for GAB type objects must be the class of variable ("ori", "spf", "dir", "siz", "col", "int"): error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).Class = class;

                    try
                        radius = str2double(attributes{2});
                    catch
                        cerror = sprintf('Second attribute for GAB type objects must be the radius: error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).Radius = radius;
 
                    try
                        angle = str2double(attributes{3});
                    catch
                        cerror = sprintf('Third attribute for GAB type objects must be the angle (of position 1): error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).Angle = angle;
                    
                    try
                        free_param = str2double(attributes{4});
                    catch
                        cerror = sprintf('Fourth attribute for GAB type objects must be the target position (1 to n): error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).FP1 = free_param;
                    
                    try
                        free_param = str2double(attributes{5});
                    catch
                        cerror = sprintf('Fifth attribute for GAB type objects must be the level of difficulty (1 to 8): error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).FP2 = free_param;
                    TaskObject(obnum).Name = 'GAB';
                    
                    
                elseif strcmp(obtype, 'msk'), 
                    %Gabor array mask: syntax: MSK(class, radius, angle)
                    %                    e.g.  MSK(ori,320,135)          
                    if numatt < 3,
                        cerror = sprintf('Must have 3 attributes (class, radius, angle) for MSK task object #%i, in condition #%i', obnum, condnum);
                        return
                    end
                 
                    class = attributes{1};
                    possibles = {'ori', 'spf', 'dir', 'siz', 'col', 'int'};  % SDS might change these to refer to types of mask 
                    
                    if ~find(strcmpi(class, possibles)==1)
                        cerror = sprintf('First attribute for MSK type objects must be the class of variable ("ori", "spf", "dir", "siz", "col", "int"): error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).Class = class;
                    ...
                    try
                        ...
                        cerror = sprintf('Second attribute for MSK type objects must be the radius: error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).Radius = ypos;
 
                    try
                        angle = str2double(attributes{3});
                    catch
                        cerror = sprintf('Third attribute for MSK type objects must be the angle (of position 1): error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).Angle = angle;  
                    TaskObject(obnum).Name = 'MSK';
                    
                                          
                elseif strcmp(obtype, 'fxc'),
                  %Fixation cross - syntax: FXC(radius, angle, size, width, colour)
                  %                   e.g.  FXC(0,0,20,4,[50 200 50]) for central cross
                  % NB. size is ratio of screen height to cross bar length
                  % NB. width is ratio of cross bar length to cross bar width                 
                    if numatt < 5,
                        cerror = sprintf('Must have 5 attributes (radius, angle, size, width, colour) for FXC task object #%i, in condition #%i', obnum, condnum);
                        return
                    end
                    ...
                    try
                        ...
                        cerror = sprintf('First attribute for FXC type objects must be the radius: error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).Radius = radius;
 
                    try
                        angle = str2double(attributes{2});
                    catch
                        cerror = sprintf('Second attribute for FXC type objects must be the angle (of position 1): error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).Angle = angle;
                    
                    try
                        free_param = str2double(attributes{3});
                    catch
                        cerror = sprintf('Third attribute for FXC type objects must be the cross size: error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).FP1 = free_param;
                    ...
                    try
                        ...
                        cerror = sprintf('Fourth attribute for FXC type objects must be the cross width: error in task object #%i,  condition #%i', obnum, condnum);
                        return
                    end
                    TaskObject(obnum).FP2 = free_param;
                    
                    rgb = eval(attributes{5});
                    if length(rgb) ~= 3 || mod(rgb(1),1) > 0 || mod(rgb(2),1) > 0 || mod(rgb(3),1) > 0
                        cerror = sprintf('Fifth attribute for FXC type objects should be [r g b] (0-255): error in task object #%i,  condition #%i', obnum, condnum);
                    end
                    TaskObject(obnum).Color = rgb;
                    TaskObject(obnum).Name = 'FXC';
                                          
%                 elseif strcmp(obtype, 'fix'), %fixation target - syntax: fix(xpos, ypos)
%     
%                     if numatt < 2,
...
%                             xpos = str2double(attributes{2});
%                         catch
%                             cerror = sprintf('Second attribute for PIC objects must be the x-position (condition #%i)', condnum); %SDS - huh? PIC object? this is obtype 'gen' !!
%                             return
%                         end
%                         TaskObject(obnum).Xpos = xpos;
%                         try
%                             ypos = str2double(attributes{3});
%                         catch
%                             cerror = sprintf('Third attribute for PIC objects must be the y-position (condition #%i)', condnum); %SDS - huh? PIC object? this is obtype 'gen' !!
%                             return
%                         end
%                         TaskObject(obnum).Ypos = ypos;
%                     else
...
%                     end
%                     try
%                         radius = str2double(attributes{1});  % SDS - NB. use of str2double for single numeric parameter
%                         rgb = eval(attributes{2});           % SDS - NB. use of eval for colour matrix parameter
...
%     
%                 elseif strcmp(obtype, 'sqr'), %square - syntax: sqr(size, rgb, fillflag, xpos, ypos)
%                     
%                     if numatt ~= 5,
...
%                     TaskObject(obnum).Name = 'Square';
                ...
                elseif strcmp(obtype, 'ttl'), %TTL pulse - syntax: ttl(outputport)
                    ...
                    cerror = sprintf('PTB only MLogic:Unrecognized task object type in condition #%i', condnum);
                end
            end
        end
