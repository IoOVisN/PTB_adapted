...
% 'acquirefix' == 0 if the joystick-position does not enter the specified radius around a target object within the allowed duration,
%                   ...otherwise == ordinal number of the acquired object, immediately this object is acquired.

% 'holdfix'    == 0 immediately, if the joystick-position exits the threshold radius around the target object before the specified duration has elapsed,
%                   ...otherwise == 1.

% 'showcursor'    controls a persistent flag within eyejoytrack; when the flag is 'on' 'acquirefix' & 'holdfix' will display cursor for as long as they are operational; 
...
% ... if an XGL target, this is the list of specified targets in the 'acquirefix' command;
...
%     chosen = items(A_TAR);
%     H_DEC = eyejoytrack('holdfix', chosen, fix_radius, hold_target_time);
%     if ~H_DEC,
%         trialerror(5); % broke fixation
