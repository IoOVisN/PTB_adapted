function varargout = PT_monkeylogic(varargin)
global MLHELPER_OFF
...
% Syntax:
%        monkeylogic(ConditionsFile, DataFile, TestFlag)  %SDS - as in L1623 'mlmenu'
%       
...
disp(sprintf('PT_MONKEY LOGIC \r PsychToolBox adapted version \r  XGL - free ! \r'))
if isempty(varargin),
...
condfile = varargin{1};   %SDS the conditions file identity is passed in the call to monkeylogic from mlmenu (RUN BUTTON) 
...
if exist(cfgfile, 'file'),
    ...
    fprintf('<<< PT_MonkeyLogic >>> Using configuration file: %s\n', cfgfile);
    xs = MLConfig.ScreenX; 
    ys = MLConfig.ScreenY;
else
...
Conditions_temp = PT_load_conditions(condfile);            %SDS  'PT_load_conditions' is a directory routine 
if iscell(Conditions_temp), %user-defined task-loop     %SDS  'iscell'  determines whether input is cell array
    numconds = 1;
...
else %standard task-loop (using a conditions text file and timing files, etc)  % SDS - runs till L207
    ...
    Txy = unique(Txy, 'rows');
    disp(sprintf('<<< PT_MonkeyLogic >>> Loaded & parsed conditions file: %s', condfile))
    drawnow;
    ...
    for i = 1:numtfiles,
        try
            ...
            disp(sprintf('<<< PT_MonkeyLogic >>> Embedding %s...', tfile));
            RunTimeFiles{i} = embedtimingfile(tfile, 'PT_trialholder.m');       % PTB amend
        catch  
            save(errorfile);
    ...
    if numtfiles == 1,
        disp('<<< PT_MonkeyLogic >>> Successfully created the run-time function from the timing script');
    else
        disp(sprintf('<<< PT_MonkeyLogic >>> Successfully created %i run-time functions from %i timing scripts...', numtfiles, numtfiles));
    end
    drawnow;
...
disp('<<< PT_MonkeyLogic >>> Initialized task parameters...')
drawnow;  % MATLAB help: "causes figure windows and their children to update, and flushes the system event queue. Any callbacks generated by incoming events (e.g., mouse or key events) are dispatched before drawnow returns"
...
if exist(codesfile, 'file'),
    ...
    if fidcodes < 0,
        ...
        disp(sprintf('<<< PT_MonkeyLogic >>> Reading behavioral codes file: %s...', codesfile))
...
disp(sprintf('<<< PT_MonkeyLogic >>> Opening %s...', datafile));
WriteData = struct([]);
if appendflag == 0,
...
bhv_write(1, fidbhv, MLConfig, condfile, RunTimeFiles, Conditions, MLConfig.EyeTransform, MLConfig.JoyTransform); %SDS 'bhvwrite' is a MLogic main directory routine
disp(sprintf('<<< PT_MonkeyLogic >>> Initialized data file %s...', datafile));
...
ScreenInfo.Xsize = xs;  %SDS set from MLConfig.ScreenX L 94
ScreenInfo.Ysize = ys;  %SDS set from MLConfig.ScreenY L 95
...
%SDS  Fixation point drawn by PsychToolBox function - this is redundant
...
ScreenInfo.ControlScreenHandle = initcontrolscreen(1, ScreenInfo, MLConfig);  %SDS  this draws the Control Screen
...
% Initialize I/O
disp(sprintf('<<< PT_MonkeyLogic >>> Starting up DAQ...'))
[DaqInfo DaqError] = initio(MLConfig.InputOutput);          %SDS 'initio' is a MLogic main directory routine 
if ~isempty(DaqError),
    daqreset;
    clear DaqInfo
...
disp(sprintf('<<< PT_MonkeyLogic >>> Successfully initialized DAQ system.'))
drawnow;
...
ScreenInfo = PT_init(ScreenInfo); % SDS subroutine at L2737
win = ScreenInfo.Ptr;
% Initialize Video
ScreenInfo.BytesPerPixel = 4;
ScreenInfo.Half_xs = round(xs/2);  % xs is ScreenInfo.Xsize L335
ScreenInfo.Half_ys = round(ys/2);  % ys is ScreenInfo.Ysize L336

% SDS X_MLVIDEO
% disp(sprintf('<<< PT_MonkeyLogic >>> Starting up video...'))
% ScreenInfo = init_video(ScreenInfo);    % SDS  'init_video' subfunction @% L2174
...
% end
% disp(sprintf('<<< PT_MonkeyLogic >>> Successfully initialized video.'))
% SDS X_MLVIDEO_end

% per MS: calculate video frame length for use with movies...
disp(sprintf('<<< PT_MonkeyLogic >>> Calculating video frame length...'))
% SDS X_MLVIDEO 
% numframes = 51;               %SDS: original monkeylogic procedure: 
...
% for i = 1:numframes  %SDS: original monkeylogic procedure replicated using Screen 'Flip' command
%     Stamp(i) = Screen('Flip',win);
% end
% ScreenInfo.FrameLength = k * mean(diff(Stamp(2:numframes))); % 

% SDS: but why not use PsychToolBox special-purpose function instead..?
ScreenInfo.FrameLength = 1000 * Screen('GetFlipInterval', win);
% SDS X_MLVIDEO_end

disp(sprintf('...Average video frame length = %2.3f ms', ScreenInfo.FrameLength))
drawnow;

% INITIALISE EYELINK  %SDS added extra
disp(sprintf('<<< PT_MonkeyLogic >>> Initialising EYELINK'))
EyeLinkInitialisation(win)
...
%     RunTimeFiles{1} = embedtimingfile(timingfile, 'PT_trialholder.m');
% end
...
% disp(sprintf('<<< PT_MonkeyLogic >>> Successfully initialized Timing Files.'))
% 
% if MLConfig.PreloadVideo,
%     [preloaded ScreenInfo.PreloadedVideoBuffers] = preload_videos(Conditions,ScreenInfo);
%     fprintf('<<< PT_MonkeyLogic >>> Successfully preloaded video files.\n');
% else
%     preloaded = [];
% end
% SDS X_MLVIDEO_end
...
if thisisonlyatest, 
    disp('<<< PT_MonkeyLogic >>> Initializing Latency Test...')
    ...
    timingfile = embedtimingfile(tfile, 'PT_trialholder.m');  % PTB amend
    ...
    disp('<<< PT_MonkeyLogic >>>  Done.')
    return
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%% TASKLOOP %%%%%%%%%%%%%%%%%%%%        Runs till 1316; loop proper starts on L622; initiates trial on line 1080
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
trialtype = 0;
ScreenInfo.UsePreProcessedImages = MLConfig.UsePreProcessedImages;
disp(sprintf('<<< PT_MonkeyLogic >>> BHV File: %s. Started running trials at %s...', datafile, datestr(rem(now, 1))))
...
for trial = 1:MLConfig.MaxTrials,   %SDS  task loop starts here; runs til  1296
    ...
    if ~userdefinedtaskloop,        %SDS   'else' @ L884, 'end' @ L 935
        %% Select Block
        if trial == 1,
            trialsperblock = 0;
            trialsthisblock = 0;
        ...
        selectednexttrial = 0;
        while selectednexttrial == 0,    %SDS  'end' @ L 877
            bswitchflag = 0;
            if pausechangeblock,
            ...
            if ~repeatflag,
                switch MLConfig.CondLogic,      %SDS this picks the condition number to implement in the upcoming trial
                    case 1 % Random with replacement
        ...
        C = Conditions(cond).TaskObject;     %SDS: Conditions is defined @ L124    %SDS: selects the TaskObject data for a single condition...
    else %userloop, as per CPS, incorporated by WA; runs to line 935
        cond = 1;
        originalcond = 1;
        block = 1;
        TrialRecord.CurrentBlock = block;
        ...
        if ~isempty(varargout), %third output is a custom version of "PT_trialholder.m"
            tholder = [MLPrefs.Directories.ExperimentDirectory varargout{1} '.m'];
        else
            tholder = 'PT_trialholder.m'; %PTB amend
        end
        try
    ...
    % Generate / Load stimuli for this trial      % SDS - 'C' is set at L 903: 'C' is TaskObject data for the single (upcoming) trial
    ...
    % Set process priority          % SDS X_MLVIDEO
    switch MLConfig.Priority,       % these commands are part of the XGL suite of functions;
        case 1,                     % they do not require any prior initialization to use
            prtnormal;              % ... and do not appear to intefere with Cogent
        case 2,
            prthigh;
        case 3,
            prtrealtime;
    ...
    prtnormal;      % SDS X_MLVIDEO - as above - (L 1047)
...
%   ScreenInfo = close_video(ScreenInfo, 'BuffersOnly'); % SDS X_MLVIDEO 
    ...
    set(findobj('tag', 'mincyclerate'), 'string', sprintf('%i Hz', TrialData.CycleRate(1)));
...
if trial == MLConfig.MaxTrials || blockcounter > MLConfig.MaxBlocks,
    ...
    monkeylogic_alert(2, '<<< PT_MonkeyLogic >>> Task Complete.', MLConfig.Alerts);
end
...
bhv_write(3, fidbhv, AllCodes, BehavioralCodes, trial);  %SDS 'bhvwrite' is a MLogic main directory routine
fclose(fidbhv);
rmpath(MLPrefs.Directories.RunTimeDirectory);
...
if trial ~= 0 && ~thisisonlyatest,
    ...
    disp(sprintf('<<< PT_MonkeyLogic >>> Finished running trials at %s.', RESULT.FinishTime))
    disp(sprintf('<<< PT_MonkeyLogic >>> Completed %i correct trials over %i total trials.', RESULT.CorrectTrials, RESULT.TotalTrials))
...
varargout = {RESULT};  %%%%%%% - END OF MONKEY LOGIC MAIN ROUTINE - %%%%%%%%
...
% SDS X_MLVIDEO     Subroutine buf_mov is now redundant 
% M = 0;
% xis = 0;
% yis = 0;
% xisbuf = 0;
...
%     if exist(file, 'file'),
%         load(file); %gets M, xis, yis, xisbuf, yisbuf  (%SDS  xis=316, yis=316, xisbuf=320, yisbuf=320)
%         numframes = size(M,2);
%         preprocessed = 1;
% 
...
% end
% if ~preprocessed,
%     reader = mmreader(name);      %SDS  looks like error - 'sourcefile' not defined anywhere; substitute 'name'
...
function [TaskObject, vbuffer, GenInfo] = create_taskobjects(C, ScreenInfo, DaqInfo, TrialRecord, mldirectories, fidbhv, varargin)
 %SDS - called from main routine L1038  as [TaskObject ScreenInfo.ActiveVideoBuffers GenInfo]= create_taskobjects (...)

%persistent GenData     Commented WA
...
GenInfo = cell(1,length(C));                %SDS - 'cell' creates a single row array of empty matrices
...
% SDS: this list of subfields for the TaskObject structure is similar, but not identical to the original list in the 'PT_load_conditions' routine
% SDS:                     'Class' and 'Modality' are new here; as are several others, e.g. 'Status'
for obnum = 1:lc,
    t = C(obnum).Type;
...
% SDS X_MLVIDEO
%     preprocessed = 0;
%   if strcmpi(t, 'fix') || strcmpi(t, 'pic') || strcmpi(t, 'dot') || strcmpi(t, 'crc') || strcmpi(t, 'sqr') || strcmpi(t, 'imd'),  % go to 1775 for 'mov'
% 
...
%         vbufnum = vbufnum + 1;
%         vbuffer(vbufnum) = vbuf;        %SDS - 'vbuffer' is returned to MLogic main routine
% 
...
% SDS X_MLVIDEO_end
                                                                                 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     elseif strcmpi(t, 'cog'), % SDS - previosly 'elseif'   % SDS cogent device      %%%%  ALL SDS ADDED EXTRAS  %%%
%         TaskObject(obnum).Class = 'Cogent';                                      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
%         TaskObject(obnum).Modality = 6; % SDS an extra modality for cogent
%         TaskObject(obnum).XPos = C(obnum).Xpos; % SDS - using Xpos for the radius parameter
%         TaskObject(obnum).YPos = C(obnum).Ypos; % SDS - using Ypos for the angle parameter
%         TaskObject(obnum).Xsize = C(obnum).Xsize; % SDS - using Xsize for target position parameter
%         TaskObject(obnum).Ysize = C(obnum).Ysize; % SDS - using Ysize for difficulty level parameter
%         TaskObject(obnum).Color = C(obnum).Color; % SDS - using Color for variable indicator
%     % SDS - These are all copied from the static objects list, above; none of these fields is included when TaskObject is initially created (L 1666)    
%         TaskObject(obnum).InitFrame = 0; %the video frame, as numbered since the beginning of a trial, in which the stimulus was first displayed
%         TaskObject(obnum).StartFrame = 1; %will remain "1" for static images (required for image translation)
%         TaskObject(obnum).FrameStep = 1; %will remain "1" for static images (required for image translation)
%         TaskObject(obnum).NumFrames = 1; %will remain "1" for static images (required for image translation)
...
%         TaskObject(obnum).NumPositions = 1; %for translation, the number of (x,y) pairs that make up the path
%         TaskObject(obnum).CurrentPosition = 1; %for translation, the index for the current (x,y) pair
%     % SDS - It's not a video object !!! 
%         TaskObject(obnum).Buffer = []; %the address of the video buffer
%     % SDS - position details are not applicable... ???  
%         TaskObject(obnum).XsPos = []; %the screen position in pixels (of XPos)
%         TaskObject(obnum).YsPos = []; %the screen position in pixels (of YPos
% 
%         TaskObject(obnum).Status = 0; %tells 'toggleobject' if the image is currently displayed
%         TaskObject(obnum).Name = C(obnum).Name;
        
%SDS: HERE  create a fresh set of instructions for each new PsychToolBox routine in the repertoire...
%SDS: NB. -also need to add fresh code in PT_load_conditions routine.

        % Firstly, require to extract screen/window size for calculating Xpos etc for certain PsychToolBox displays(e.g. FXC, ...,)
        [w h] = Screen('WindowSize', ScreenInfo.Ptr);  % Alternatively, screenInfo must have current screen dimensions ...)

    if strcmpi(t, 'gab'),
        TaskObject(obnum).Class = C(obnum).Class;
        TaskObject(obnum).Modality = 1; %SDS: leave modality ==2 as a spare setting, for future development... 
        TaskObject(obnum).Radius = C(obnum).Radius; % SDS - using Xpos for the radius parameter
        TaskObject(obnum).Angle = C(obnum).Angle; % SDS - using Ypos for the angle parameter
        TaskObject(obnum).FP1 = C(obnum).FP1; % SDS - FP1 is the target position parameter
        TaskObject(obnum).FP2 = C(obnum).FP2; % SDS - FP2 is the difficulty level parameter
        TaskObject(obnum).Color = [];
        TaskObject(obnum).Status = 0; %tells 'toggleobject' if the image is currently displayed
        TaskObject(obnum).Name = C(obnum).Name; % .Name is set in PT_load_conditions as 'GAB'
        
        TaskObject(obnum).XPos = 0; %the screen position in degrees (origin at screen centre); replaced by vector of gabor centre positions during trial initialization
        TaskObject(obnum).YPos = 0; %the screen position in degrees (origin at screen centre); replaced by vector of gabor centre positions during trial initialization
        TaskObject(obnum).Xsize = 100;%2*C(obnum).Radius;  %the screen size in pixels; replaced by vector of gabor sizes during trial initialization
        TaskObject(obnum).Ysize = 200;%*C(obnum).Radius;  %the screen size in pixels; replaced by vector of gabor sizes during trial initialization
            
    elseif strcmpi(t, 'msk'),
        TaskObject(obnum).Class = C(obnum).Class;
        TaskObject(obnum).Modality = 1; %SDS: leave modality ==2 as a spare setting, for future development... 
        TaskObject(obnum).Radius = C(obnum).Radius; % SDS - using Xpos for the radius parameter
        TaskObject(obnum).Angle = C(obnum).Angle; % SDS - using Ypos for the angle parameter
        TaskObject(obnum).FP1 = [];
        TaskObject(obnum).FP2 = [];
        TaskObject(obnum).Color = [];
        TaskObject(obnum).Status = 0; %tells 'toggleobject' if the image is currently displayed
        TaskObject(obnum).Name = C(obnum).Name;  % .Name is set in PT_load_conditions as 'MSK'
        
        TaskObject(obnum).XPos = 0; %the screen position in degrees (origin at screen centre)
        TaskObject(obnum).YPos = 0; %the screen position in degrees (origin at screen centre)
        TaskObject(obnum).Xsize = 2*C(obnum).Radius;  %the screen size in pixels
        TaskObject(obnum).Ysize = 2*C(obnum).Radius;  %the screen size in pixels
        
    elseif strcmpi(t, 'fxc'),
        TaskObject(obnum).Class = C(obnum).Class;
        TaskObject(obnum).Modality = 1; %SDS: leave modality ==2 as a spare setting, for future development... 
        TaskObject(obnum).Radius = C(obnum).Radius; 
        TaskObject(obnum).Angle = C(obnum).Angle; 
        TaskObject(obnum).FP1 = C(obnum).FP1; % SDS - FP1 is the cross bar-length parameter
        TaskObject(obnum).FP2 = C(obnum).FP2; % SDS - FP2 is the cross bar-width parameter
        TaskObject(obnum).Color = C(obnum).Color; % SDS - Color is an rgb triplet [0-255]
        TaskObject(obnum).Status = 0; %tells 'toggleobject' if the image is currently displayed
        TaskObject(obnum).Name = C(obnum).Name;  % .Name is set in PT_load_conditions as 'FXC'
        
        radius = C(obnum).Radius*h/200;  % radius in condition file for FXC device is set as percentage of half screen height (pixels)
        angle = C(obnum).Angle;         % angle clockwise from 12 O'clock (degrees)
        angle = angle *pi/180;          % angle clockwise from 12 O'clock (radians)
        size = floor(h/C(obnum).FP1);   % cross length in in condition file for FXC device is set as ratio of screen height to cross barlength
        
        TaskObject(obnum).XPos = radius * sin(angle); %the screen position in degrees 
        TaskObject(obnum).YPos = radius * cos(angle); %the screen position in degrees
        TaskObject(obnum).Xsize = size;  %the screen size in pixels
        TaskObject(obnum).Ysize = size;  %the screen size in pixels
...
% SDS X_MLVIDEO     subroutine init_video is now redundant
ScreenInfo.IsActive = 0;
try
    mlvideo('init');
...
% 
% % Initialize Photodiode trigger:    %SDS YETOODO
...
function ScreenInfo = PT_init(ScreenInfo)

% screenid = 3-ScreenInfo.Device;         % SDS: wierdly, MLOGIC(XGL) & PTB refer to screens in different ways
screenid = ScreenInfo.Device; 

oldRes = SetResolution(screenid,ScreenInfo.Xsize,ScreenInfo.Ysize);
 % Malfunctions on XP with window 1 (large screen) and all resolutions
 % ... other than 1680 1050; also generates java errors; 
 % OK with window 2 (small screen)

%  if ~ScreenInfo.Xsize == 1680
%         disp('error: PTB screen malfunctions if screen resolution NOT 1680 1050 \n [this error generated by L2749 in PT_MonkeyLogic main routine')
%         SetResolution(screenid, oldres)
%         return
% end
...
% screenid = 3-SI.Device;         % SDS: wierdly, MLOGIC(XGL) & PTB refer to screens in different ways
...
function EyeLinkInitialisation(win)

% return  %SDS temporary inactivation !!!
% global stopkey eye_used el edfFile Options
% 
% if Options.Eye_Tracker_mode~=1
%     return
% end

if EyelinkInit()~= 1; % SDS: no arguments; default..  dummymode ==0 & enablecallbacks ==1
    return;
end;

% Eyelink('Command', 'screen_pixel_coords = -512, 384, 512, -384'); %SDS Colin prog

el=EyelinkInitDefaults(win);  

el.foregroundcolour = [256,256,256];                  %SDS Colin prog
el.backgroundcolour = [128,128,128];                        %SDS Colin prog

if ~isempty(el.callback) %#ok<AND2>                 %SDS Colin prog
    PsychEyelinkDispatchCallback(el);               %SDS Colin prog
end
...
Eyelink('Command', 'link_sample_data = LEFT,RIGHT,GAZE,AREA');              %SDS: PTB 'example'  

NameOfData = date;                                                            %SDS Colin prog
edfFile = strcat(NameOfData(1:2),NameOfData(4:6),NameOfData(10:11),'.edf');   %SDS Colin prog

Eyelink('Openfile', edfFile);

% EyelinkExample step 4
...
EyeLinkCoordonees(-1) %initializes the eye coordinate reading routine, that includes the following pair of assignments:
