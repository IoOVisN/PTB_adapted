...
% Modified July 2012 SDS: to accomodate PsychToolBox 
%       No essential modification; may reassign modality ==2 @L255/270 (currrently  movie)
...
if procnum == 1, %WA create window
    ...
    xs = 800;
    ys = 600;
    xratio = xs/ScreenInfo.Xsize; %WA these are needed to scale objects
    ...
    blankstring = '-';
    blankstring = repmat(blankstring, 1, 26); %WA number of zeros which can fit on this line
...
%     h(21) = uicontrol('style', 'text', 'position', [xs+cp3size+22 ys+cp2size-341 83 20], 'string', '', 'foregroundcolor', [1 1 1], 'fontweight', 'bold', 'fontsize', 11, 'horizontalalignment', 'center');
    ...
    h(20) = uicontrol('style', 'text', 'position', [xs+cp3size+20 ys+cp2size-322 90 20], 'string', 'Min Cycle-Rate:', 'backgroundcolor', fbgc);
    ...
    h(21) = uicontrol('style', 'text', 'position', [xs+cp3size+22 ys+cp2size-341 83 20], 'string', ' ---- Hz', 'tag', 'mincyclerate', 'foregroundcolor', [1 1 1], 'fontweight', 'bold', 'fontsize', 10, 'horizontalalignment', 'center', 'backgroundcolor', [.3 .3 .3]);
%     switch MLConfig.Priority,                          % Commented WA
%         case 1,
...
%     end
    ...
    %WA User text box.  Fits 20 chars per line on 12 lines.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% REWARD
    rtext = text(x1, y1+0.8, 'Reward Duration: 120');
    set(rtext, 'color', legendcol);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %WA: timeline axis
    fcolor = [0.5 0.5 0.7];
    %WA: subplot('position', [(cp3size/20)/(cp3size+xs+cpsize) cp2size/(cp2size+ys+cp4size) (cp3size*18/20)/(cp3size+xs+cpsize) ys/(cp2size+ys+cp4size)]);
    ...
    output = csh; %WA: return figure window handle

elseif procnum == 2, %WA: update window with task objects

    fig = findobj('tag', 'mlmonitor');  
    figure(fig);
    xyratio = get(gcf, 'userdata'); %  xyratio  == [800/ScreenX 600/ScreenY],  [0.7813 0.7813] for 1024 x 768 screen; [0.4762 0.5714] for 1680 x 1050 screen
    ...
    for obnum = 1:numobjects,
        Ob = TaskObject(obnum);
        if Ob.Modality == 1 || Ob.Modality == 2 % SDS modality 1 is now assigned to PsychToolBox devices
            x = Ob.XPos(1);
            y = Ob.YPos(1);
            xsize = Ob.Xsize(1);
            ysize = Ob.Ysize(1);
            ...
            xyr = xyratio(where);
            msize = mx*xyr/ScreenInfo.PixelsPerPoint; % ScreenInfo.PixelsPerPoint == 1.333; hence mx*0.4762/1.333   == mx*0.3572
            h = plot(x, y, 'square');
            ...
            set(h, 'xdata', ScreenInfo.OutOfBounds, 'ydata', ScreenInfo.OutOfBounds); %WA: off screen
            TaskObject(obnum).ControlObjectHandle = h;
%             if Ob.Modality == 2, %WA: movie,
%                 set(h, 'linewidth', 4, 'markeredgecolor', [0 1 0]);
%             end
        elseif Ob.Modality == 4 || Ob.Modality == 5, %WA: analog stimulation or TTL
            if Ob.Modality == 5,
                obcol = [1 0.6 0.6];
            else
                obcol = [1 1 0.4];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %WA: create joystick target circle:
    ...
    %WA: create button indicators
    numbuttons = 5; %WA: max, for now at least
    ...
    %WA: set(gca, 'color', [0 0 0]);
    output = TaskObject; %WA: return expanded TrialObject structure

elseif procnum == 3, %WA: update performance bars

    perf = ScreenInfo; %WA: expected to be three column vectors with trial-errors 0 through n; first column is over-all, second is this block, third this condition
...
elseif procnum == 4, %WA: update timeline
...
elseif procnum == 5, %WA: update RT graphs
...
elseif procnum == 6, %WA: update user text
...
elseif procnum == 7, %WA: update warning text
...
for k = 1:.1:10,
    count = count + 1;
	y = exp(sin(t.*k));
	refreshdata(h,'caller') %WA: Evaluate y in the function workspace
    tic
	drawnow
    z(count) = toc;
end
